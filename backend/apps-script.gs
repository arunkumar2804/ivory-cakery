...
const SCRIPT_VERSION = '2026-05-13-crm-invoice-v4';
...
function testNotifications() {
  ...
  const result = {
    version: SCRIPT_VERSION,
    ownerEmailSent: ownerResult.ownerEmailSent,
    telegramSent: ownerResult.telegramSent,
    customerEmailSent: customerEmailSent,
    errors: ownerResult.errors
  };
  Logger.log('testNotifications result: ' + JSON.stringify(result));
  return result;
}
...
function handleStatusChange(e) {
  ...
  try {
    if (status === "preparing") {
      ...
      ss.toast(`Email & Invoice sent for ${name}`, "✅ Success");
    } 
      ...
      ss.toast(`Ready notification sent to ${name}`, "✅ Success");
    }
      ...
      ss.toast(`Completion email sent to ${name}`, "✅ Success");
    }
      ...
      ss.toast(`Cancellation email sent to ${name}`, "⚠️ Alert");
    }
  } catch (err) {
