...
const MSG91_TEMPLATE_ID = 'YOUR_TEMPLATE_ID'; // Create a Flow in Msg91 first
const SMS_PROVIDER      = 'fast2sms'; // Set to 'msg91' or 'fast2sms'const TELEGRAM_BOT_TOKEN = '8624014283:AAEq_4dFpWV0wETpd9YplQ2j4VsCQzH7jvc';
...
const SCRIPT_VERSION = '2026-05-14-crm-v11-msg91-ready';
...
function doPost(e) {
  ...
  try {
    ...
    if (phone && phone !== 'No Phone') {
      try {
        const note = message || '';
        const variables = { name: name, cakeType: cakeType, orderId: orderId, status: 'Received', note: note };
        const smsMsg = getStatusMessage(name, cakeType, orderId, 'New', note);
        sendSMS(phone, smsMsg, variables);
        result.smsSent = true;
        Logger.log('SMS sent.');
      } catch (err) {
...
function handleStatusChange(e) {
  ...
  const phone  = data[3];
  const note   = data[8];
  const variables = { name: name, cakeType: type, orderId: orderId, status: statusRaw, note: note };
  const smsMsg = getStatusMessage(name, type, orderId, statusRaw, note);
  
  if (phone) sendSMS(phone, smsMsg, variables);
    if (status === "preparing") {
...
/** 11. UNIFIED SMS SERVICE */
function sendSMS(phoneNumber, message, variables) {
  if (SMS_PROVIDER === 'msg91') {
    return sendMsg91(phoneNumber, variables);
  } else {
    return sendFast2SMS(phoneNumber, message);
  }
}

/** 11a. Fast2SMS SERVICE */
function sendFast2SMS(phoneNumber, message) {
  ...
  try {
    ...
    if (result.return === true) {
      Logger.log('SMS sent successfully via Fast2SMS to ' + phoneNumber);
    } else {
      Logger.log('Fast2SMS failed: ' + result.message);
    }
  } catch (e) {
...
/** 11b. Msg91 SERVICE */
function sendMsg91(phoneNumber, vars) {
  if (!MSG91_AUTH_KEY || MSG91_AUTH_KEY === 'YOUR_MSG91_AUTH_KEY') {
    Logger.log('Msg91 Auth Key missing.');
    return;
  }

  const url = "https://api.msg91.com/api/v5/flow/";
  
  // Format variables for Msg91 (key-value pairs)
  // Ensure your Flow uses these exact variable names
  const payload = {
    "template_id": MSG91_TEMPLATE_ID,
    "short_url": "0",
    "recipients": [
      {
        "mobiles": phoneNumber.toString().replace(/\+/g, '').replace(/\s/g, ''),
        "name": vars.name,
        "cake": vars.cakeType,
        "orderid": vars.orderId,
        "status": vars.status,
        "note": vars.note || "Have a wonderful day!"
      }
    ]
  };
  ...
  try {
    const response = UrlFetchApp.fetch(url, options);
    Logger.log('Msg91 Response: ' + response.getContentText());
  } catch (e) {
    Logger.log('Msg91 error: ' + e.toString());
  }
}
